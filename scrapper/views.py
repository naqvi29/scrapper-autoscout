        ...
        if request.session['is_login'] is True:
            if request.method =='POST':
                ...
                while True:
                    ...
                    items =  soup.select('div.ListItem_header__uPzec')
                    ...
                    if r.status_code != 200:
                        break
                    num = [int(s) for s in soup.select_one('h1').text if s.isdigit()][0]
                    if num == 0:
                        break
                    if "0" in soup.select_one('h1').text:
                        break
                    num = [int(s) for s in soup.select_one('h1').text if s.isdigit()][0]
                    if num == 0:
                        break
                ...
                for url in urls:
                    ...
                    try:
                        name =soup2.select_one('div > h1').text.strip()
                        alt = name.split()[0]
                    except:
                        name =''
                    try:
                        # price =soup2.select_one('span.css-1b5kt8d').text.strip()
                        price = re.findall(r'€ [0-9,\.]+,-',r.text)[0]
                        # price = price.replace('€','')
                        # price = price.replace(' ','')
                        # price = price.replace(',','')
                        # price = price.replace('-','')
                        ...
                        price =''
                    try:
                        # kilometers =soup2.select_one('div.css-o8htdy:nth-of-type(1) div.css-16ceglb').text.strip()
                        kilometers =re.findall(r'[0-9,\.]+ km',r.text)[0]
                    ...
                    try:
                        ...
                        # image =soup2.select_one('source[type="image/jpeg"]').get('srcset')
                        image =soup2.select_one('picture img').get('src')
                    except:
                        image =''

                    try:
                        # power =soup2.select_one('div.css-o8htdy:nth-of-type(5) div.css-16ceglb').text.strip().replace('\n',' ')
                        power =re.findall(r'[0-9]+ kW \([0-9]+ CV\)',r.text)[0]
                    except:
                        power =''
                    try:
                        # date =soup2.select_one('div.css-o8htdy:nth-of-type(3) div.css-16ceglb').text
                        date =re.findall(r'[0-9]+/[0-9]+',r.text)[0]
                    except:
                        date =''

                    number =soup2.select_one('a.css-15fzge')
                    # number = re.findall(r'\+[0-9]+ [0-9]+ - [0-9]+',r.text)
...
def scrapper_bot_send_via_number_single(request,number):
    ...
    auth_token = '9daf268b64c86778b4853dfdb9639b8a'
